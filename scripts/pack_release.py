# -*- coding: utf-8 -*-
"""打包 dist/Workbench -> dist/Workbench-<version>.zip（zipfile，兼容 extract_pyinstaller_zip.py）。
...
SRC = os.path.join(ROOT, "dist", "Workbench")
version = sys.argv[1] if len(sys.argv) > 1 else "vX.Y.Z"
OUT = os.path.join(ROOT, "dist", f"Workbench-{version}.zip")
