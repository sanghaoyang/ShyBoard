...
- 快捷方式：常用网站一键直达
- 统计：各状态任务数、今日完成数
- 日历：左侧菜单 📅 切换月历视图，每天显示农历日期 + 节日/节气标识（国庆、清明、24 节气等），支持任意年月跳转；待办任务的截止日期会在日历上标注（过期红色/未过期粉色），点击某天可写当天记录（日记）
- 日记：点击日历某天 → 文本框记录当天发生的事，独立存储不进任务列表（「保存记录」或底部「确定」均可保存）
- 纪念日：记录阳历或农历纪念日（如生日、中秋，闰月自动处理），集成在日历内——日历下方列表 + 倒计时 + 日历格子标记；「＋ 纪念日」可指定日期，点某天可固定当天
- 主题：粉色 / 白色 / 黑色 / 橙色 / 绿色五套色系一键切换（设置 → 界面主题），各主题独立协调色调，字体颜色随主题明暗联动保证可读性
