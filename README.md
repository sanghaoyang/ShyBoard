...
- 便签与快捷方式
- 天气、番茄钟和多套界面主题
- 摸鱼小馆：数织（最高 25×25，生成题经过唯一解、纯逻辑与难度分级验证）、2048、翻牌配对、经典扫雷和点灯；各游戏均有难度梯度与本机个人排行榜
- 内存友好：小游戏按需加载，Nonogram 求解缓存用后释放，后台不可见时暂停轮询
...
- 在“设置 → 数据迁移”中选择“导出数据”，新环境中再选择“导入数据”。任务、状态、进度历史、便签、快捷方式、纪念日、每日记录、定时任务和界面偏好会一并恢复。
- 也可以在 ShyBoard 完全退出后复制整个应用目录。

导入数据会替换当前工作台内容。ShyBoard 会在导入前自动保存当前数据库快照，放在 `data/backups/` 中。
