...
async function loadStats() {
  try {
    ...
    const active = Number(s.todo || 0) + Number(s.doing || 0);
    $("#stats").innerHTML = `
      <div class="stat-cell"><div class="stat-num">${active}</div><div class="stat-label">正在进行</div></div>
      <div class="stat-cell done"><div class="stat-num">${s.done}</div><div class="stat-label">累计完成</div></div>
...
function renderTasks(tasks) {
  ...
  $("#cnt-doing").textContent = active.length;
}
...
function openCreateTaskModal() {
  ...
  $("#modal-body").innerHTML = `
    <div class="create-task-form">
      ...
      <input id="create-task-tags" type="text" placeholder="用逗号分隔，例如：工作, 本周">
    </div>`;
  openModal();
